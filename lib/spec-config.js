...
export function buildCmds(projectDir, opts) {
  ...
  const audit = runner === 'pnpm exec'
    ...
    : 'npm audit --omit=dev';
  const extraIgnore = env.SPEC_PRETTIER_IGNORE ? env.SPEC_PRETTIER_IGNORE.split(',').map((p) => `"!${p.trim()}"`).join(' ') : '';
  const prettier = (cmd) => `${runner} prettier --config .configs/.prettierrc --ignore-path .configs/.prettierignore --${cmd} "src/**/*.{js,css}" "!src/vendor/**" "!src/icons.json"${extraIgnore ? ' ' + extraIgnore : ''} scripts`;
